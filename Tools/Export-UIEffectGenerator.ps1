...
$packageRoot = Join-Path $projectRoot 'Packages/LxyGame/LxyGame.UIEffectGenerator'
...
if ([string]::IsNullOrWhiteSpace($Destination)) {
    $Destination = Join-Path $projectRoot "Build/UPMPackages/$($manifest.name)-$($manifest.version)"
}
...
Write-Output "Exported $($manifest.name) $($manifest.version) to: $destinationPath"
