...
Lua 源码始终维护在项目根目录 `Lua`。YooAsset Bundle Builder 开始构建时
会临时把它们转换为 `Assets/GameResources/Lua/**/*.lua.bytes`；该目录已经被
`DefaultPackage` 的 YooAsset Collector 收集，构建结束后会自动删除，不能作为
常驻资源目录使用：
...
直接 Player 构建不会同步 Lua。使用 YooAsset 的 Bundle Builder 时，点击
Build 会先临时同步 Lua，随后执行原有 Bundle 构建，完成或失败后都会清理。
