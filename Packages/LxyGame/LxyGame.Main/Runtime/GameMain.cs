...
namespace LxyDemo
{
    ...
    public class GameMain : MonoBehaviour
    {
        ...
        private const string SceneRuntimeAssemblyName =
            "Game.Logic";

        private const string SceneRuntimeTypeName =
            "LxyDemo.SceneManagement.GameSceneManager";
        ...
        private ISceneRuntime sceneRuntime;
        ...
        {
            if (!TryCreateSceneRuntime(out string sceneRuntimeError))
            {
                Fail(sceneRuntimeError);
                yield break;
            }
            ...
            SetStage(
                ...
                0.75f,
                $"加载业务场景：{targetScene}");
            sceneRuntime.SceneLoadProgressChanged +=
            ...
            Debug.Log(
                ...
                this);
            Coroutine operation =
                sceneRuntime.LoadSceneAsync(targetScene);
            if (operation != null)
            {
            ...
            if (!string.IsNullOrEmpty(sceneRuntime.LastError))
            {
                Fail(sceneRuntime.LastError);
                yield break;
            }

            if (!string.Equals(
                    sceneRuntime.ActiveSceneName,
                    targetScene,
                    StringComparison.Ordinal))
            {
                Fail(
                    $"业务场景加载结果不一致，期望 {targetScene}，" +
                    $"实际 {sceneRuntime.ActiveSceneName}。");
                yield break;
            }
        ...
        /// <summary>
        /// 从已加载的 Game.Logic 中创建场景服务。AOT 启动壳只依赖
        /// Game.Contracts 中的接口，不引用具体游戏逻辑程序集。
        /// </summary>
        private bool TryCreateSceneRuntime(out string error)
        {
            error = null;
            ...
            Type runtimeType = runtimeAssembly.GetType(
                SceneRuntimeTypeName);
            if (runtimeType == null ||
                !typeof(MonoBehaviour).IsAssignableFrom(runtimeType) ||
                !typeof(ISceneRuntime).IsAssignableFrom(runtimeType))
            {
                error = "场景服务未实现 ISceneRuntime：" +
                        SceneRuntimeTypeName;
                return false;
            }

            var runtimeObject = new GameObject("[GameSceneManager]");
            sceneRuntime = runtimeObject.AddComponent(runtimeType)
                as ISceneRuntime;
            if (sceneRuntime != null)
            {
                return true;
            ...
            error = "无法创建场景服务：" + SceneRuntimeTypeName;
            return false;
        }

        /// <summary>
        /// 从已加载的热更新程序集动态创建首场景运行时。
        /// Login 是 APK 内置场景，若把 Game.Logic 的 MonoBehaviour 直接
