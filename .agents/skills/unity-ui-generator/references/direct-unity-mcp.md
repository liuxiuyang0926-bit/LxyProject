...
- 不修改 `Packages/com.lxy.ui-effect-generator` 核心生成器、`Packages/com.lxy.ui` 项目适配器、Skill、asmdef、MCP 包或现有业务逻辑。
...
7. 写入目标 Schema 后，使用 UnityMCP 导入它，并通过 `unity_execute_code` 调用 `Lxy.UIEffectGenerator.Editor.UIEffectPrefabBuilder.GenerateFromSchemaPath(...)`。传入提示中的 `panelId`、`prefabFolder`、脚本设置、`UIEffectResourceMatchMode.VisualSimilarity` 与资源搜索根；Builder 会通过 LxyDemo 已注册适配器保留 `CSharpUIGenerator` 的 Canvas、Binder 和代码流水线。
